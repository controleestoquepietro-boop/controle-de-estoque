...
export default function Dashboard() {
  ...
  useEffect(() => {
    console.log(
      "Dashboard: importDialogOpen state changed ->",
      importDialogOpen,
    );
  }, [importDialogOpen]);
  const [alimentoParaSaida, setAlimentoParaSaida] =
    useState<AlimentoComputado | null>(null);
    ...
    queryFn: async () => {
      try {
        ...
        try {
          ...
          if (profileError) {
            console.warn(
              "Não foi possível buscar perfil na tabela users:",
              profileError,
            );
          } else if (profile) {
            ...
            (authUser.user_metadata.nome || authUser.user_metadata.full_name)
          ) {
  ...
  const { data: alimentos = [], isLoading: alimentosLoading } = useQuery<
    Alimento[]
  >({
    queryKey: ["alimentos"],
    queryFn: async () => {
      try {
        ...
        return data.map((alimento: any) => ({
          ...alimento,
          dataFabricacao: alimento.dataFabricacao
            ? new Date(alimento.dataFabricacao).toISOString().split("T")[0]
            : null,
          dataValidade: alimento.dataValidade
            ? new Date(alimento.dataValidade).toISOString().split("T")[0]
            : null,
        }));
      } catch (error) {
  ...
  const saidaMutation = useMutation({
    mutationFn: async ({
      ...
      return await apiRequest("POST", `/api/alimentos/${id}/saida`, {
        quantidade,
      });
    },
    onSuccess: () => {
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <div className="flex gap-2 flex-wrap items-center relative">
              <Collapsible
                ...
                onOpenChange={setFileActionsOpen}
              >
                <CollapsibleTrigger asChild>
                  ...
                  >
                    ...
                    Gerenciar Arquivos
                    <ChevronDown
                      className={`h-4 w-4 transition-transform ${fileActionsOpen ? "rotate-180" : ""}`}
                    />
                  </Button>
                </CollapsibleTrigger>
                ...
                {fileActionsOpen && (
                  <CollapsibleContent className="absolute right-0 top-full translate-y-2 bg-card border rounded-md shadow-lg p-2 z-10 flex flex-col gap-2 min-w-[200px]">
                    <Button
                      ...
                      size="sm"
                      onClick={() => {
                        console.log("Dashboard: clique menu Importar Modelos");
                    ...
                    <Button
                      ...
                      onClick={() => {
                        console.log(
                          "Dashboard: clique menu Importar Alimentos",
                        );
