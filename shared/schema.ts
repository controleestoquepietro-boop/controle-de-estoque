...
export const insertUserSchema = createInsertSchema(users, {
  ...
  // 'color' é gerado pelo servidor no momento do registro
  color: true,
});

// Schema de modelo de produto
export const insertModeloProdutoSchema = createInsertSchema(modelosProdutos).extend({
  ...
  pesoEmbalagem: z.number().nullable(),
  pesoPorCaixa: z.number().nullable(),
  empresa: z.string().nullable(),
  pesoLiquido: z.number().nullable(),
  tipoPeso: z.string().nullable(),
  quantidadePorCaixa: z.number().int().nullable(),
  unidadePadrao: z.enum(["kg", "caixa"]).default("kg"),
  // cadastradoPor pode ser preenchido pelo servidor quando a importação for feita
  cadastradoPor: z.string().min(1, "Cadastrado por é obrigatório").optional(),
});

// Schema de alimento SEM departamento
export const insertAlimentoSchema = createInsertSchema(alimentos, {
...
export type ModeloProduto = typeof modelosProdutos.$inferSelect;
...
// Tipos para o frontend (com campos computados)
export interface AlimentoComputado extends Alimento {
  dataInspecao: string;
  status: 'VENCIDO' | 'VENCE EM BREVE' | 'ATIVO' | 'AGUARDANDO_CADASTRO';
